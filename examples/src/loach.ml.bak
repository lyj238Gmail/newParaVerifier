...
let cmpStrengthPrPInv pinvs ~types paramRef pr=		
	let Paramfix(pname,indextype,Intc(index))=paramRef in
	let mkActualParasForInv pinv=
		let parasOfPInv=pinv in
		let paraNumPairs=List.zip parasOfPInv List.upto 0 (List.length parasOfPInv) in
			List.map
			 ~f:(fun (p,num)->let Paramref(pname,indextype)=p in Paramfix(pname,indextype,num+index))
			 paraNumPairs
	let List.map ～f:(fun ind->paramfix pname indexType (Intc(ind)) [index,index+1] in
