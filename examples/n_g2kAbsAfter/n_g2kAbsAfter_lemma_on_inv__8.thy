(*  Title:      HOL/Auth/n_g2kAbsAfter_lemma_on_inv__8.thy
...
theory n_g2kAbsAfter_lemma_on_inv__8 imports n_g2kAbsAfter_base
begin
section{*All lemmas on causal relation between inv__8 and some rule r*}
lemma n_n_SendInvAck_i1Vsinv__8:
...
lemma n_n_RecvInvAck_i1Vsinv__8:
...
have "((formEval (eqn (IVar (Ident ''ExGntd'')) (Const true)) s))\<or>((formEval (neg (eqn (IVar (Ident ''ExGntd'')) (Const true))) s))" by auto
  moreover {
    ...
    have "?P3 s"
    apply (cut_tac a1 a2  c1, simp, rule_tac x="(neg (andForm (eqn (IVar (Field (Ident ''Chan3_1'') ''Cmd'')) (Const InvAck)) (eqn (IVar (Field (Ident ''Cache_1'') ''State'')) (Const E))))" in exI, auto) done
...
lemma n_n_ARecvInvAck_i1Vsinv__8:
...
have "((formEval (eqn (IVar (Ident ''ExGntd'')) (Const true)) s))\<or>((formEval (neg (eqn (IVar (Ident ''ExGntd'')) (Const true))) s))" by auto
  moreover {
    assume c1: "((formEval (eqn (IVar (Ident ''ExGntd'')) (Const true)) s))"
    have "?P3 s"
    apply (cut_tac a1 a2  c1, simp, rule_tac x="(neg (andForm (eqn (IVar (Field (Ident ''AChan3_1'') ''Cmd'')) (Const InvAck)) (eqn (IVar (Field (Ident ''Cache_1'') ''State'')) (Const E))))" in exI, auto) done
    then have "invHoldForRule s f r (invariants N)" by auto
  }
  moreover {
    assume c1: "((formEval (neg (eqn (IVar (Ident ''ExGntd'')) (Const true))) s))"
...
lemma n_n_ASendGntE_i1Vsinv__8:
...
lemma n_n_SendReqEI_i1Vsinv__8:
...
lemma n_n_ASendReqEI_i1Vsinv__8:
...
lemma n_n_ASendReqIS_j1Vsinv__8:
...
lemma n_n_ASendReqES_i1Vsinv__8:
...
lemma n_n_ARecvGntE_i1Vsinv__8:
...
lemma n_n_ASendGntS_i1Vsinv__8:
...
lemma n_n_ARecvGntS_i1Vsinv__8:
...
lemma n_n_ASendInvE_i1Vsinv__8:
...
lemma n_n_ASendInvS_i1Vsinv__8:
...
lemma n_n_SendReqES_i1Vsinv__8:
...
lemma n_n_ASendReqSE_j1Vsinv__8:
...
lemma n_n_SendReqEE_i1Vsinv__8:
...
lemma n_n_ASendInvAck_i1Vsinv__8:
