...
let () = run_with_cmdline (fun () ->
  ...
  (*let bad_states = StateGen.bfs inits rules 2000 in
  print_endline ("bad:\n"^String.concat ~sep:"\n" (List.map bad_states ~f:ToStr.Smv.form_act)^sprintf "\n%d\n" (List.length bad_states));*)
  let good_states =
    let rec wrapper s =
      match s with
    ...
    in
    FwdStateGen.fwdBfs (simplify (wrapper init)) rules 2000
  in
  print_endline ("good:\n"^String.concat ~sep:"\n" (List.map good_states ~f:ToStr.Smv.form_act)^sprintf "\n%d\n" (List.length good_states))
)
