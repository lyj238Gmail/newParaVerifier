...
rule "n_SendGntE_i1"
  ((((((CurCmd = ReqE) & (CurPtr = 1)) & (ExGntd = false)) & (forall j : NODE do (ShrSet[j] = false) endforall)) & (forall j : AUX_NODE do (AShrSet[j] = false) endforall)) & (Chan2_1.Cmd = Empty)) ==>
begin
  ShrSet_1 := true;
  CurCmd := Empty;
  ExGntd := true;
...
rule "n_ASendGntE_i1"
  ((((((CurCmd = ReqE) & (CurPtr = 1)) & (ExGntd = false)) & (forall j : AUX_NODE do (AShrSet[j] = false) endforall)) & (forall j : NODE do (ShrSet[j] = false) endforall)) & (AChan2_1.Cmd = Empty)) ==>
begin
  AShrSet_1 := true;
  CurCmd := Empty;
  ExGntd := true;
...
invariant "n_DataProp"
  (((ExGntd = false) -> (MemData = AuxData)) & (forall i : NODE do ((!(Cache_1.State = I)) -> (Cache_1.Data = AuxData)) endforall));


invariant "n_DataProp1"
  (((ExGntd = false) -> (MemData = AuxData)) & (forall i : AUX_NODE do ((!(ACache_1.State = I)) -> (ACache_1.Data = AuxData)) endforall));
