...
let n_Try =
  ...
  let formula = (andList [(eqn (var (arr [("n", [paramref "i"])])) (const _I)); (eqn (param (paramref "i")) (const (intc 1)))]) in
...
let n_Crit =
  let name = "n_Crit" in
  let params = [paramdef "i" "client"] in
  let formula = (andList [(andList [(eqn (var (arr [("n", [paramref "i"])])) (const _T)); (eqn (var (global "x")) (const (boolc true)))]); (eqn (param (paramref "i")) (const (intc 1)))]) in
...
let n_Exit =
  let name = "n_Exit" in
  let params = [paramdef "i" "client"] in
  let formula = (andList [(eqn (var (arr [("n", [paramref "i"])])) (const _C)); (eqn (param (paramref "i")) (const (intc 1)))]) in
...
let n_Idle =
  let name = "n_Idle" in
  let params = [paramdef "i" "client"] in
  let formula = (andList [(eqn (var (arr [("n", [paramref "i"])])) (const _E)); (eqn (param (paramref "i")) (const (intc 1)))]) in
...
let () = run_with_cmdline (fun () ->
  ...
  let cinvs_with_varnames, relations = find protocol
    ~murphi:(In_channel.read_all "n_mutualExOnI.m")
