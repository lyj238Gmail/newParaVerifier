(*  Title:      HOL/Auth/n_germanSimp_lemma_on_inv__11.thy
...
theory n_germanSimp_lemma_on_inv__11 imports n_germanSimp_base
begin
...
moreover {
  assume b1: "(i~=p__Inv4)"
  have "?P3 s"
  apply (cut_tac a1 a2 b1, simp, rule_tac x="(neg (andForm (neg (eqn (IVar (Field (Para (Ident ''Cache'') i) ''State'')) (Const I))) (eqn (IVar (Field (Para (Ident ''Chan2'') p__Inv4) ''Cmd'')) (Const GntE))))" in exI, auto) done
  then have "invHoldForRule s f r (invariants N)" by auto
}
...
lemma n_RecvReqE__part__0Vsinv__11:
...
lemma n_RecvReqE__part__1Vsinv__11:
