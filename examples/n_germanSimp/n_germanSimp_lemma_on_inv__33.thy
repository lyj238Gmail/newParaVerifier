(*  Title:      HOL/Auth/n_germanSimp_lemma_on_inv__33.thy
...
theory n_germanSimp_lemma_on_inv__33 imports n_germanSimp_base
begin
...
lemma n_RecvReqE__part__0Vsinv__33:
...
lemma n_RecvReqE__part__1Vsinv__33:
...
moreover {
  assume b1: "(i~=p__Inv4)"
  have "?P3 s"
  apply (cut_tac a1 a2 b1, simp, rule_tac x="(neg (andForm (andForm (andForm (andForm (eqn (IVar (Field (Para (Ident ''Chan2'') p__Inv4) ''Cmd'')) (Const Empty)) (eqn (IVar (Para (Ident ''ShrSet'') p__Inv4)) (Const true))) (eqn (IVar (Field (Para (Ident ''Cache'') p__Inv4) ''State'')) (Const I))) (eqn (IVar (Ident ''CurCmd'')) (Const ReqS))) (eqn (IVar (Ident ''ExGntd'')) (Const false))))" in exI, auto) done
  then have "invHoldForRule s f r (invariants N)" by auto
}
ultimately show "invHoldForRule s f r (invariants N)" by satx
qed
