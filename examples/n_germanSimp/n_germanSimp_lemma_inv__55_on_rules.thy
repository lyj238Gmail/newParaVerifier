(*  Title:      HOL/Auth/n_germanSimp_lemma_inv__55_on_rules.thy
...
theory n_germanSimp_lemma_inv__55_on_rules imports n_germanSimp_lemma_on_inv__55
begin
section{*All lemmas on causal relation between inv__55*}
lemma lemma_inv__55_on_rules:
  assumes b1: "r \<in> rules N" and b2: "(\<exists> p__Inv3 p__Inv4. p__Inv3\<le>N\<and>p__Inv4\<le>N\<and>p__Inv3~=p__Inv4\<and>f=inv__55  p__Inv3 p__Inv4)"
  ...
  apply (cut_tac b1, auto) done
    ...
    moreover {
      ...
      have "invHoldForRule s f r (invariants N)"
      apply (cut_tac b2 d1, metis n_RecvReqE__part__0Vsinv__55) done
    }

    moreover {
      assume d1: "(\<exists> i. i\<le>N\<and>r=n_RecvReqE__part__1 N i)"
      have "invHoldForRule s f r (invariants N)"
      apply (cut_tac b2 d1, metis n_RecvReqE__part__1Vsinv__55) done
    }
