(*  Title:      HOL/Auth/n_mutualExFsm_lemma_on_inv__1.thy
...
theory n_mutualExFsm_lemma_on_inv__1 imports n_mutualExFsm_base
begin
...
moreover {
  ...
  moreover {
    ...
    have "?P3 s"
    apply (cut_tac a1 a2 b1 c1, simp, rule_tac x="(neg (andForm (eqn (IVar (Para (Ident ''n'') p__Inv1)) (Const C)) (eqn (IVar (Ident ''x'')) (Const true))))" in exI, auto) done
    then have "invHoldForRule s f r (invariants N)" by auto
  }
  moreover {
