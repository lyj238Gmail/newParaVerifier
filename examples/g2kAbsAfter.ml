...
let n_n_SendGntE_i1 =
  ...
  let params = [] in
  let formula = (andList [(andList [(andList [(andList [(andList [(eqn (var (global "CurCmd")) (const _ReqE)); (eqn (var (global "CurPtr")) (const (intc 1)))]); (eqn (var (global "ExGntd")) (const (boolc false)))]); (eqn (var (global "ShrSet_1")) (const (boolc false)))]); (eqn (var (global "AShrSet_1")) (const (boolc false)))]); (eqn (var (record [global "Chan2_1"; global "Cmd"])) (const _Empty))]) in
...
let n_n_ASendGntE_i1 =
  ...
  let params = [] in
  let formula = (andList [(andList [(andList [(andList [(andList [(eqn (var (global "CurCmd")) (const _ReqE)); (eqn (var (global "CurPtr")) (const (intc 1)))]); (eqn (var (global "ExGntd")) (const (boolc false)))]); (eqn (var (global "ShrSet_1")) (const (boolc false)))]); (eqn (var (global "AShrSet_1")) (const (boolc false)))]); (eqn (var (record [global "AChan2_1"; global "Cmd"])) (const _Empty))]) in
...
let n_n_DataProp =
  ...
  let params = [] in
  let formula = (andList [(imply (eqn (var (global "ExGntd")) (const (boolc false))) (eqn (var (global "MemData")) (var (global "AuxData")))); (imply (neg (eqn (var (record [global "Cache_1"; global "State"])) (const _I))) (eqn (var (record [global "Cache_1"; global "Data"])) (var (global "AuxData"))))]) in
  prop name params formula

let n_n_DataProp1 =
  let name = "n_n_DataProp1" in
  let params = [] in
  let formula = (andList [(imply (eqn (var (global "ExGntd")) (const (boolc false))) (eqn (var (global "MemData")) (var (global "AuxData")))); (imply (neg (eqn (var (record [global "ACache_1"; global "State"])) (const _I))) (eqn (var (record [global "ACache_1"; global "Data"])) (var (global "AuxData"))))]) in
  prop name params formula
...
let () = run_with_cmdline (fun () ->
  ...
  let cinvs_with_varnames, relations = find protocol
    ~murphi:(In_channel.read_all "n_g2kAbsAfter.m")
  in
