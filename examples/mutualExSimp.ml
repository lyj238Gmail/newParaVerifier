...
let n_Crit =
  ...
  let formula = (andList [(eqn (var (arr [("n", [paramref "i"])])) (const _I)); (eqn (var (global "x")) (const (boolc true)))]) in
...
let rules = [n_Crit; n_Exit; n_Idle]
...
let () = run_with_cmdline (fun () ->
  ...
  let cinvs_with_varnames, relations = find protocol
    ~murphi:(In_channel.read_all "n_mutualExSimp.m")
  in
