...
var
  ...
  auxdata: array [addr_type] of data_type;
	globalMem: array[addr_type] of data_type;
...
begin
  ...
  then
    if node[home].cache[addr].state = cache_shared then
	  ...
	  for i: num_data_type do
	    node[home].home_requests[addr].data.values[i]   := globalMem[addr].values[i];
	  endfor;
    endif;
...
begin
  if node[home].directory[addr][source] = cache_exclusive then
	for i: num_data_type do
      globalMem[addr].values[i] := node[home].inchan[channel3].msg.data.values[i];
	endfor;
  endif;
  for i: num_data_type do
